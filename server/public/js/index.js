...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _interface = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./lottery/interface.js\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
	...
	var Calculate = function () {
	    ...
	    _createClass(Calculate, [{
	        ...
	        value: function computeCount(active, play_name) {
	            var count = 0; //当前注数
	            var exist = this.play_list.has(play_name); //判断玩法列表是否存在
	            var arr = new Array(active).fill('0'); //生成数组对数组进行计算组合运算 fill填充0
	            if (exist && play_name.at(0) === 'r') {
	                //如果支持 并 玩法支持等于r ==>R2 R3 R4....
	        ...
	        value: function computeBonus(active, play_name) {
	            var play = play_name.split(''); //当前玩法 R2 分割 R 和2
	            var self = this;
	            var arr = new Array(play[1] * 1).fill(0); //R2 就是生成数组arr[0,1]个，填充为0
	            var min = void 0,
	                max = void 0;
	            if (play[0] === 'r') {
	            ...
	            (function f(arr, size, result) {
	                ...
	                if (size === arrLen) {
	                    ...
	                    for (var i = 0; i < arrLen; i++) {
	                        var newResult = [].concat(result); //concat() 方法用于连接两个或多个数组
	                        newResult.push(arr[i]);
	                        if (size === 1) {
	                            allResult.push(newResult);
	                        } else {
	                            var newArr = [].concat(arr);
	                            newArr.splice(0, i + 1); //splice() 方法向/从数组中添加/删除项目，然后返回被删除的项目。
